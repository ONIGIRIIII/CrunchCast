...
with:
  - difficulty_score: the proxy-for-workload label, a composite of that
    ...
    percentile-ranked WITHIN course_level (100/200/.../600) rather than
    across the whole catalog - see add_difficulty_label's docstring for why
    (short version: grad/professional courses grade far more generously,
    so a global ranking makes almost every intro course look "hard").
...
def add_course_level(df: pd.DataFrame) -> pd.DataFrame:
    """course_level ("100", "200", ... "0" if unparsable) from the leading
    digits of the course number. Computed early, before the label/component
    scores below, because those are now percentile-ranked WITHIN course
    level rather than across the whole catalog - see add_difficulty_label.
    """
    df = df.copy()
    leading_digits = df["course"].str.extract(r"^(\d+)")[0]
    course_num = pd.to_numeric(leading_digits, errors="coerce").fillna(0).astype(int)
    df["course_level"] = (course_num // 100 * 100).astype(str)
    return df
...
def add_difficulty_label(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Built from z-scores of (100 - avg), fail_rate, and std_dev, THEN
    percentile-ranked WITHIN course_level (not across the whole catalog).

    Why within-level: the catalog is dominated by generously-graded
    grad/professional courses (mean difficulty ~18-20 for 500/600-level vs.
    ~70 for 100-level - checked directly on this dataset). Ranking a 100-level
    course against that whole pool means almost every popular intro course
    looks "hard" by construction, which isn't a useful comparison for a
    student choosing between courses at the same level. Ranking within level
    means a course's score reflects how it compares to its actual peers.
    ...
    """
    df = df.copy()
    grp = df.groupby("course_level")
    avg_z = (df["avg"] - grp["avg"].transform("mean")) / grp["avg"].transform("std")
    fail_z = (df["fail_rate"] - grp["fail_rate"].transform("mean")) / grp["fail_rate"].transform("std")
    std_z = (df["std_dev"] - grp["std_dev"].transform("mean")) / grp["std_dev"].transform("std")
    composite = (-avg_z + fail_z + std_z) / 3
    df[LABEL_COL] = composite.groupby(df["course_level"]).rank(pct=True) * 100
    return df
...
def add_component_scores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    within-course_level percentile-rank recipe as difficulty_score (see its
    docstring for why), applied to one raw column each instead of a
    z-scored composite.
    ...
    Also ranked within course_level: a 300-person 100-level lecture and a
    300-person 400-level lecture are both "big for their level" in a way a
    single global class-size ranking would muddy.
    """
    df = df.copy()
    grp = df.groupby("course_level")
    df["grade_score"] = (-df["avg"]).groupby(df["course_level"]).rank(pct=True) * 100
    df["failrisk_score"] = grp["fail_rate"].rank(pct=True) * 100
    df["variance_score"] = grp["std_dev"].rank(pct=True) * 100
    df["classsize_score"] = grp["enrolled"].rank(pct=True) * 100
    return df
...
def add_credits(df: pd.DataFrame, metadata: StaticCSVMetadataProvider) -> pd.DataFrame:
    df = df.copy()
...
def main():
    ...
    sections = add_course_level(sections)
    ...
    sections = add_credits(sections, metadata)
