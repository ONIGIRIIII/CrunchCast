...
export default function CourseSearch({ onSelectCourse, addedKeys, atMax }: Props) {
  ...
  return (
    <div className="relative">
      <label className="block text-xs font-medium text-[var(--color-text-subtle)] mb-1.5" htmlFor="course-search">
        Search courses
      </label>
      <input
        id="course-search"
        ...
        className="w-full rounded-md border border-[var(--color-border-strong)] bg-transparent px-3.5 py-2.5 text-sm disabled:opacity-40 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500"
        autoComplete="off"
      />
      ...
      {open && query.trim() && (
        <ul className="absolute z-30 mt-1.5 w-full max-h-72 overflow-y-auto rounded-md border border-[var(--color-border)] bg-[var(--color-surface-raised)] shadow-lg">
          ...
          {!loading && !error && matches.length === 0 && (
            <li className="px-3.5 py-2.5 text-sm text-[var(--color-text-subtle)]">No matches - try the fields below.</li>
          )}
