...
def main():
    ...
    lines.append(
        ...
        "- difficulty_score is percentile-ranked WITHIN course_level "
        "(100/200/.../600), not across the whole catalog: a score of 80 "
        "means 'harder than about 80% of OTHER COURSES AT THIS LEVEL,' not "
        "an absolute workload amount, and not a comparison to, say, a "
        "600-level seminar. This was a deliberate fix - an earlier global-"
        "percentile version made nearly every 100-level course look 'hard' "
        "purely because the catalog is full of generously-graded grad "
        "courses, which was a useless comparison for a student picking "
        "between intro courses. See data/README.md and 'Error by course "
        "level' above (now roughly even across levels, as intended - the "
        "MAE is higher than a global ranking would report, which is the "
        "honest cost of removing an easy-to-predict but not-useful signal).\n"
        ...
        "- The worst individual predictions above still skew toward small, "
        "specialized cohorts (grad/professional-program-flavored subjects "
        "recur). The likely reason: small, homogeneous cohorts can swing "
        "the percentile-ranked score from near 0 to near 100 with almost "
        "no change in the raw average grade, and the model - trained on "
        "that course's typically-calm history - has no way to see it "
        "coming. This is a real limitation of the proxy label's "
        "sensitivity for small cohorts, not a generic ML failure.\n"
    )
